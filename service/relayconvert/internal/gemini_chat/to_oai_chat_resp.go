...
func UsageFromGeminiMetadata(metadata *dto.GeminiUsageMetadata, fallbackPromptTokens int) *dto.Usage {
	...
	promptTokens := max(metadata.PromptTokenCount, 0) + max(metadata.ToolUsePromptTokenCount, 0)
	...
	usage := &dto.Usage{
		...
		CompletionTokens: max(metadata.CandidatesTokenCount, 0) + max(metadata.ThoughtsTokenCount, 0),
	...
	usage.CompletionTokenDetails.ReasoningTokens = max(metadata.ThoughtsTokenCount, 0)
	usage.PromptTokensDetails.CachedTokens = max(metadata.CachedContentTokenCount, 0)

	for _, detail := range metadata.PromptTokensDetails {
		if detail.Modality == "AUDIO" {
			usage.PromptTokensDetails.AudioTokens += max(detail.TokenCount, 0)
		} else if detail.Modality == "IMAGE" {
			usage.PromptTokensDetails.ImageTokens += max(detail.TokenCount, 0)
		} else if detail.Modality == "TEXT" {
			usage.PromptTokensDetails.TextTokens += max(detail.TokenCount, 0)
		}
	}
	...
	for _, detail := range metadata.CandidatesTokensDetails {
		...
		case "IMAGE":
			usage.CompletionTokenDetails.ImageTokens += max(detail.TokenCount, 0)
		case "AUDIO":
			usage.CompletionTokenDetails.AudioTokens += max(detail.TokenCount, 0)
		case "TEXT":
			usage.CompletionTokenDetails.TextTokens += max(detail.TokenCount, 0)
		}
	}
