...
func TestUsageFromResponsesUsagePreservesTokenDetails(t *testing.T) {
	...
	require.Equal(t, 6, usage.PromptTokensDetails.AudioTokens)
	require.Equal(t, 70, usage.CompletionTokenDetails.TextTokens)
	require.Equal(t, 8, usage.CompletionTokenDetails.ImageTokens)
	require.Equal(t, 9, usage.CompletionTokenDetails.AudioTokens)
...
func TestResponsesResponseToChatCompletionsNormalizesEmptyFunctionArguments(t *testing.T) {
	tests := []struct {
		name      string
		arguments []byte
	}{
		{name: "missing", arguments: nil},
		{name: "empty string", arguments: []byte(`""`)},
		{name: "blank string", arguments: []byte(`"   "`)},
