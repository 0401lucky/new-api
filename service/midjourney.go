...
// PrepareMidjourneyTaskBilling 在任务入库前设置持久化退款标记。
...
// SettleMidjourneyTaskBilling 对已持久化的旧版任务扣费，并记录实际完成的阶段。
func SettleMidjourneyTaskBilling(relayInfo *relaycommon.RelayInfo, task *model.Midjourney, prepared bool) (bool, error) {
	...
	task.TemporaryQuotaConsumed = relayInfo.TemporaryQuotaConsumed
	task.PermanentQuotaConsumed = relayInfo.PermanentQuotaConsumed
	task.SetTemporaryAllocations(relayAllocationsToModel(relayInfo.TemporaryQuotaAllocations))
	if updateErr := task.UpdateBillingState(); updateErr != nil {
...
// RefundMidjourneyQuota 回退旧版任务已记录的全部计费数据。
func RefundMidjourneyQuota(ctx context.Context, task *model.Midjourney, reason string) bool {
	...
	if _, err := model.RefundWallet(task.UserId, quota, &model.WalletSplit{
	...
	task.Quota = 0
	task.TemporaryQuotaConsumed = 0
	task.PermanentQuotaConsumed = 0
	task.SetTemporaryAllocations(nil)
