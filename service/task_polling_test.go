...
func TestSweepUnrefundedFailedTasksRefundsModernTaskAndSkipsLegacyAndNegative(t *testing.T) {
	truncate(t)
	...
	negativeTask := makeTask(userID, 0, -700, 0, BillingSourceWallet, 0)
	negativeTask.TaskID = "negative_failed_without_refund"
	negativeTask.Status = model.TaskStatusFailure
	negativeTask.Progress = "100%"
	negativeTask.SubmitTime = model.TaskRefundLegacyCutoff
	negativeTask.UpdatedAt = time.Now().Add(-time.Minute).Unix()
	require.NoError(t, model.DB.Create(negativeTask).Error)
	...
	var reloadedNegative model.Task
	...
	require.NoError(t, model.DB.First(&reloadedNegative, negativeTask.ID).Error)
	assert.Zero(t, reloadedModern.Quota)
	assert.Equal(t, legacyTaskQuota, reloadedLegacy.Quota)
	assert.Equal(t, -700, reloadedNegative.Quota)
