...
func GenerateTextOtherInfo(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, modelRatio, groupRatio, completionRatio float64,
	...
	if relayInfo.PriceData.GroupRatioInfo.DynamicRatio > 0 {
		other.SetPublic("dynamic_ratio", relayInfo.PriceData.GroupRatioInfo.DynamicRatio)
		other.SetPublic("group_ratio", relayInfo.PriceData.GroupRatioInfo.GroupRatio/relayInfo.PriceData.GroupRatioInfo.DynamicRatio)
...
func appendDynamicRatioMatchInfo(other *model.LogOther, info hosttypes.GroupRatioInfo) {
	...
	if info.DynamicRatioRuleId > 0 {
		other.SetPublic("dynamic_ratio_rule_id", info.DynamicRatioRuleId)
	}
	other.SetPublic("dynamic_ratio_balance_quota", info.DynamicRatioBalanceQuota)
	if info.DynamicRatioBalanceMinQuota != nil {
		other.SetPublic("dynamic_ratio_balance_min_quota", info.DynamicRatioBalanceMinQuota)
	}
	if info.DynamicRatioBalanceMaxQuota != nil {
		other.SetPublic("dynamic_ratio_balance_max_quota", info.DynamicRatioBalanceMaxQuota)
	}
}
	...
	if relayInfo.BillingSource == "subscription" {
		...
		if walletConsumed > 0 {
			...
			if relayInfo.TemporaryQuotaConsumed > 0 {
				other.SetPublic("temporary_quota_consumed", relayInfo.TemporaryQuotaConsumed)
				if relayInfo.TemporaryQuotaCheckinId != 0 {
					other.SetPublic("temporary_quota_checkin_id", relayInfo.TemporaryQuotaCheckinId)
				}
			}
		}
	}
...
func GenerateMjOtherInfo(relayInfo *relaycommon.RelayInfo, priceData hosttypes.PriceData) *model.LogOther {
	...
	if priceData.GroupRatioInfo.DynamicRatio > 0 {
		other.SetPublic("dynamic_ratio", priceData.GroupRatioInfo.DynamicRatio)
		other.SetPublic("group_ratio", priceData.GroupRatioInfo.GroupRatio/priceData.GroupRatioInfo.DynamicRatio)
