...
func TestRefundTaskQuota_NegativeQuotaIsRejected(t *testing.T) {
	truncate(t)
	ctx := context.Background()

	const userID = 6
	seedUser(t, userID, 5000)
	task := makeTask(userID, 0, -1200, 0, BillingSourceWallet, 0)
	task.Status = model.TaskStatusFailure
	require.NoError(t, model.DB.Create(task).Error)

	assert.False(t, RefundTaskQuota(ctx, task, "invalid negative quota"))
	assert.Equal(t, 5000, getUserQuota(t, userID))
	assert.Equal(t, -1200, getTaskQuota(t, task.ID))
	assert.Equal(t, int64(0), countLogs(t))
}
