...
// 所有展示时间均按签到时区（北京时间）由后端格式化，前端不自行换算浏览器时区。
type CheckinTimeInfo struct {
	...
	Timezone             string `json:"timezone"`               // 签到时区（固定北京时间）
	CurrentDate          string `json:"current_date"`           // 北京时间今天的日期 YYYY-MM-DD
	...
	AvailableFromDisplay string `json:"available_from_display"` // 开放时间展示（HH:mm，签到时区）
	ExpiresAtDisplay     string `json:"expires_at_display"`     // 失效时间展示（MM-DD HH:mm，签到时区）
}

// ComputeCheckinTimeInfo 计算签到时间领域数据。
// 所有日期、开放时间、失效时间均使用签到专用北京时间。
// checkedToday 为 true 表示用户今天已签到。
func ComputeCheckinTimeInfo(userId int, checkedToday bool) (*CheckinTimeInfo, error) {
	return ComputeCheckinTimeInfoAt(userId, common.NowInCheckinTimezone(), checkedToday)
}
...
func ComputeCheckinTimeInfoAt(userId int, now time.Time, checkedToday bool) (*CheckinTimeInfo, error) {
	...
	info := &CheckinTimeInfo{
		...
		Timezone:             common.CheckinTimezoneName(),
