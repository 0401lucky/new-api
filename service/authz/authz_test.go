...
func TestInitMigratesLegacyAdminPermissionsOnce(t *testing.T) {
	db := newAuthzTestDB(t)
	require.NoError(t, db.AutoMigrate(&model.User{}, &model.Option{}))

	legacyDefault := model.User{Id: 101, Username: "legacy-default", Password: "hashed", Role: common.RoleAdminUser, AffCode: "legacy-101"}
	legacyCustom := model.User{Id: 102, Username: "legacy-custom", Password: "hashed", Role: common.RoleAdminUser, AffCode: "legacy-102"}
	settings := legacyCustom.GetSetting()
	...
	legacyCustom.SetSetting(settings)
	require.NoError(t, db.Create(&legacyDefault).Error)
	require.NoError(t, db.Create(&legacyCustom).Error)

	require.NoError(t, Init(db))
	assert.True(t, Can(legacyDefault.Id, common.RoleAdminUser, ChannelSensitiveWrite))
	assert.True(t, Can(legacyDefault.Id, common.RoleAdminUser, ChannelSecretView))
	assert.False(t, Can(legacyCustom.Id, common.RoleAdminUser, ChannelOperate))
	assert.False(t, Can(legacyCustom.Id, common.RoleAdminUser, ChannelSensitiveWrite))
	assert.True(t, Can(legacyCustom.Id, common.RoleAdminUser, ChannelSecretView))

	require.NoError(t, SetUserPermissions(legacyDefault.Id, PermissionsMap{ResourceChannel: {
		ActionRead:           true,
		ActionOperate:        true,
		ActionWrite:          true,
		ActionSensitiveWrite: false,
	...
	require.NoError(t, Init(db))
	assert.False(t, Can(legacyDefault.Id, common.RoleAdminUser, ChannelSensitiveWrite))
	assert.False(t, Can(legacyDefault.Id, common.RoleAdminUser, ChannelSecretView))
}
