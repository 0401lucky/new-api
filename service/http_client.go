...
// NormalizeProxyURL 按运行时兼容规则校验代理地址并返回规范缓存键。
...
// ValidateProxyURL 校验渠道代理地址，不建立网络连接。
...
// GetHttpClientWithProxy 返回默认客户端或缓存的代理客户端。
...
// InvalidateProxyClient 删除一个代理客户端并关闭其空闲连接。
...
// ResetProxyClientCache 清空代理客户端缓存。
...
// NewProxyHttpClient 为兼容旧调用保留。
// Deprecated: 请使用 GetHttpClientWithProxy。
