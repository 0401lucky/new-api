...
func TestConvertOpenAIResponsesRequestToGeminiNormalizesEmptyFunctionArguments(t *testing.T) {
	tests := []struct {
		name      string
		arguments any
		include   bool
		...
		{name: "empty", arguments: "", include: true},
		{name: "whitespace", arguments: "   ", include: true},
		{name: "null", arguments: nil, include: true},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.include {
				functionCall["arguments"] = tt.arguments
			}

			got := mustConvertResponsesToGemini(t, dto.OpenAIResponsesRequest{
				Model: "gemini-test",
				Input: mustGeminiRawMessage(t, []map[string]any{functionCall}),
			})

			require.Len(t, got.Contents, 1)
			require.Len(t, got.Contents[0].Parts, 1)
			require.NotNil(t, got.Contents[0].Parts[0].FunctionCall)
			assert.Equal(t, map[string]interface{}{}, got.Contents[0].Parts[0].FunctionCall.Arguments)
		})
	}
}
