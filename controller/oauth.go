...
func GenerateOAuthCode(c *gin.Context) {
	...
	if oauth.GetProvider(request.Provider) == nil ||
		...
		(request.Intent != model.AuthFlowIntentLogin && request.InviteCode != "") ||
	...
	userID := 0
	sessionID := ""
	flowPayload := oauthFlowPayload{AffiliateCode: request.Aff, InvitationCode: request.InviteCode}
...
func findOrCreateOAuthUser(c *gin.Context, provider oauth.Provider, oauthUser *oauth.OAuthUser, affiliateCode string, invitationCode string) (*model.User, error) {
	user := &model.User{}
