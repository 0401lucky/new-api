...
func TestTaskSubmissionPreservesTemporaryWalletFundingThroughPluginFailure(t *testing.T) {
	events := []string{}
	database := setupTaskSubmissionDatabase(t, true, &events)
	require.NoError(t, database.AutoMigrate(&model.User{}, &model.Checkin{}, &model.Log{}, &model.Channel{}))
	...
	user := model.User{Id: 1, Username: "plugin-wallet", Quota: 1000, Status: common.UserStatusEnabled}
	require.NoError(t, database.Create(&user).Error)
	now := common.NowInCheckinTimezone().Unix()
	first := model.Checkin{UserId: 1, CheckinDate: "2026-08-08", QuotaType: model.CheckinQuotaTypeTemporary, QuotaAwarded: 600, QuotaRemaining: 600, QuotaExpiresAt: now + 3600}
	second := model.Checkin{UserId: 1, CheckinDate: "2026-08-09", QuotaType: model.CheckinQuotaTypeTemporary, QuotaAwarded: 400, QuotaRemaining: 400, QuotaExpiresAt: now + 7200}
	...
	info.PriceData.GroupRatioInfo = types.GroupRatioInfo{GroupRatio: 2, DynamicRatio: 2, DynamicRatioRuleId: 17}
	billing, billingErr := service.NewBillingSession(c, info, 1200)
	...
	outcome, taskErr := executeTaskSubmissionWith(c, info, func(*gin.Context, *relaycommon.RelayInfo) (*relay.TaskSubmitResult, *dto.TaskError) {
		return &relay.TaskSubmitResult{UpstreamTaskID: "upstream_wallet", Platform: constant.TaskPlatform("plugin"), Quota: 650}, nil
	})
	require.Nil(t, taskErr)
	require.NotNil(t, outcome)
	var persisted model.Task
	require.NoError(t, database.First(&persisted, outcome.Task.ID).Error)
	assert.Equal(t, 650, persisted.PrivateData.TemporaryQuotaConsumed)
	assert.Zero(t, persisted.PrivateData.PermanentQuotaConsumed)
	require.Len(t, persisted.PrivateData.TemporaryAllocations, 2)
	assert.Equal(t, 600, persisted.PrivateData.TemporaryAllocations[0].Amount)
	assert.Equal(t, 50, persisted.PrivateData.TemporaryAllocations[1].Amount)
	require.NotNil(t, persisted.PrivateData.BillingContext)
	assert.EqualValues(t, 17, persisted.PrivateData.BillingContext.DynamicRatioRuleId)

	require.NoError(t, database.Model(&persisted).Update("status", model.TaskStatusFailure).Error)
	persisted.Status = model.TaskStatusFailure
	require.True(t, service.RefundTaskQuota(context.Background(), &persisted, "plugin failed"))
	require.NoError(t, database.First(&first, first.Id).Error)
	require.NoError(t, database.First(&second, second.Id).Error)
	require.NoError(t, database.First(&user, user.Id).Error)
	assert.Equal(t, 600, first.QuotaRemaining)
	assert.Equal(t, 400, second.QuotaRemaining)
...
func setupTaskSubmissionDatabase(t *testing.T, migrate bool, events *[]string) *gorm.DB {
	...
	require.NoError(t, database.Callback().Create().Before("gorm:create").Register("test:task-submit-order", func(tx *gorm.DB) {
		if _, task := tx.Statement.Dest.(*model.Task); task {
			*events = append(*events, "insert")
		}
	}))
	if migrate {
		require.NoError(t, database.AutoMigrate(&model.Task{}, &model.User{}, &model.Channel{}, &model.Log{}, &model.ModelHealthSlice5m{}, &model.Checkin{}))
	}
...
func TestImmediateTaskSettlementDatabase(t *testing.T) {
	...
	models := []any{&model.User{}, &model.Channel{}, &model.Task{}, &model.Log{}, &model.Checkin{}}
