...
func UpdateChannelStatus(c *gin.Context) {
	...
	updated, message, err := updateSingleChannelStatus(id, req.Status)
	if err != nil {
		common.ApiError(c, err)
		return
	}
...
func BatchUpdateChannelStatus(c *gin.Context) {
	...
	c.JSON(http.StatusOK, gin.H{
		"success": attempted > 0 && changedCount == attempted,
		"message": "",
		"data":    changedCount,
	})
}
...
func CopyChannel(c *gin.Context) {
	...
	if err := clone.ValidateSettings(); err != nil {
		...
		c.JSON(http.StatusOK, gin.H{"success": false, "message": "复制渠道失败：代理设置无效"})
		return
	}
