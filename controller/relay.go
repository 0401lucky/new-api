...
func recordLeakProtectionBlockedLog(c *gin.Context, reason string) {
	...
	other.SetAdmin("reject_reason", "leak_protection")
	other.SetPublic("leak_protection_reason", reason)
	other.SetPublic("leak_protection_balanced", true)
...
func executeTaskSubmissionWith(
	...
	if billing, ok := relayInfo.Billing.(*service.BillingSession); ok {
		if split, wallet := billing.WalletFundingSnapshot(result.Quota); wallet {
			task.PrivateData.TemporaryQuotaConsumed = split.Temporary
			task.PrivateData.PermanentQuotaConsumed = split.Permanent
			task.PrivateData.TemporaryQuotaCheckinId = split.LastCheckinId()
			task.PrivateData.TemporaryQuotaExpiresAt = split.LastExpiresAt()
			task.PrivateData.TemporaryAllocations = split.Allocations
		}
	}
