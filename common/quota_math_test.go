...
func TestQuotaRound(t *testing.T) {
	...
	assert.Equal(t, MinQuota, QuotaRound(math.Inf(-1)))
...
func TestQuotaFromFloatChecked(t *testing.T) {
	...
	require.NotNil(t, clamp)
	assert.Equal(t, QuotaClampUnderflow, clamp.Kind)
	assert.Equal(t, "-Inf", clamp.AuditMap()["original"])
	...
	assert.Equal(t, "NaN", clamp.AuditMap()["original"])
	_, err := Marshal(clamp.AuditMap())
