...
type DynamicRatioRule struct {
	...
	BalanceMinQuota *int64  `json:"balance_min_quota" gorm:"column:balance_min_quota"`
	BalanceMaxQuota *int64  `json:"balance_max_quota" gorm:"column:balance_max_quota"`
...
func (r *DynamicRatioRule) Validate() error {
	...
	if r.BalanceMinQuota != nil && *r.BalanceMinQuota < 0 {
		return fmt.Errorf("余额下限不能为负数")
	}
	if r.BalanceMaxQuota != nil && *r.BalanceMaxQuota <= 0 {
		return fmt.Errorf("余额上限必须大于 0")
	}
	if r.BalanceMinQuota != nil && r.BalanceMaxQuota != nil && *r.BalanceMaxQuota <= *r.BalanceMinQuota {
		return fmt.Errorf("余额上限必须大于余额下限")
	}
...
func UpdateDynamicRatioRule(rule *DynamicRatioRule) error {
	...
	result := DB.Model(&DynamicRatioRule{}).Where("id = ?", rule.Id).Updates(map[string]interface{}{
		...
		"balance_min_quota": rule.BalanceMinQuota,
		"balance_max_quota": rule.BalanceMaxQuota,
...
type DynamicRatioStatus struct {
	...
	ActiveRuleId          int64                 `json:"active_rule_id,omitempty"`
	BalanceQuota          int64                 `json:"balance_quota"`
	ActiveBalanceMinQuota *int64                `json:"active_balance_min_quota,omitempty"`
	ActiveBalanceMaxQuota *int64                `json:"active_balance_max_quota,omitempty"`
...
type DynamicRatioSummary struct {
	...
	BalanceMinQuota *int64  `json:"balance_min_quota"`
	BalanceMaxQuota *int64  `json:"balance_max_quota"`
...
type DynamicRatioMatch struct {
	Ratio           float64 `json:"ratio"`
	RuleId          int64   `json:"rule_id"`
	BalanceQuota    int64   `json:"balance_quota"`
	BalanceMinQuota *int64  `json:"balance_min_quota,omitempty"`
	BalanceMaxQuota *int64  `json:"balance_max_quota,omitempty"`
}

func GetDynamicRatioStatus(group string) DynamicRatioStatus {
	return GetDynamicRatioStatusWithBalance(group, 0)
}

func GetDynamicRatioStatusWithBalance(group string, balanceQuota int64) DynamicRatioStatus {
	return GetDynamicRatioStatusForGroupsWithBalance([]string{group}, balanceQuota)
}

func GetDynamicRatioStatusForGroups(groups []string) DynamicRatioStatus {
	return GetDynamicRatioStatusForGroupsWithBalance(groups, 0)
}

func GetDynamicRatioStatusForGroupsWithBalance(groups []string, balanceQuota int64) DynamicRatioStatus {
	...
	for _, r := range groupRules {
		status.Rules = append(status.Rules, DynamicRatioSummary{
			...
			BalanceMinQuota: r.BalanceMinQuota,
			BalanceMaxQuota: r.BalanceMaxQuota,
			Weekdays:        r.Weekdays,
	...
	for _, group := range groups {
		activeMatch := matchDynamicRatioMatch(rules, group, "", concurrency, now, balanceQuota, false)
		if activeMatch.Ratio <= 0 {
			continue
		}
		if !hasActiveRatio || math.Abs(activeMatch.Ratio-1) > math.Abs(status.ActiveRatio-1) {
			status.ActiveRatio = activeMatch.Ratio
			status.ActiveGroup = group
			status.ActiveRuleId = activeMatch.RuleId
			status.ActiveBalanceMinQuota = activeMatch.BalanceMinQuota
			status.ActiveBalanceMaxQuota = activeMatch.BalanceMaxQuota
			hasActiveRatio = true
		}
	}
...
func GetMatchedDynamicRatio(group string, modelName string) float64 {
	return GetMatchedDynamicRatioMatch(group, modelName, 0).Ratio
}
...
func GetMatchedDynamicRatioMatch(group string, modelName string, balanceQuota int64) DynamicRatioMatch {
	...
	return matchDynamicRatioMatch(rules, group, modelName, getActiveConnections(), common.NowInStartupTimezone(), balanceQuota, true)
}
...
func matchDynamicRatio(rules []parsedDynamicRatioRule, group string, modelName string, concurrency int64, now time.Time) float64 {
	return matchDynamicRatioMatch(rules, group, modelName, concurrency, now, 0, true).Ratio
}

func matchDynamicRatioMatch(rules []parsedDynamicRatioRule, group string, modelName string, concurrency int64, now time.Time, balanceQuota int64, respectModel bool) DynamicRatioMatch {
	type scoredRule struct {
		rule            parsedDynamicRatioRule
		hasConcurrency  bool
		concurrencyGap  int64
		hasBalanceRange bool
		hasModel        bool
	}
	...
	for _, r := range rules {
		...
		modelMatched := false
		if respectModel && r.ParsedModels != nil {
		...
		if !matchBalanceRange(r, balanceQuota) {
			continue
		}
		...
		sr := scoredRule{
			...
			hasBalanceRange: r.BalanceMinQuota != nil || r.BalanceMaxQuota != nil,
			hasModel:        respectModel && r.ParsedModels != nil,
		}
	...
	for _, m := range matched[1:] {
		if m.hasModel && !best.hasModel {
			...
			continue
		} else if m.hasBalanceRange && !best.hasBalanceRange {
			best = m
		} else if !m.hasBalanceRange && best.hasBalanceRange {
			continue
		} else if m.hasConcurrency && best.hasConcurrency {
	...
	return DynamicRatioMatch{
		Ratio:           best.rule.Ratio,
		RuleId:          best.rule.Id,
		BalanceQuota:    balanceQuota,
		BalanceMinQuota: best.rule.BalanceMinQuota,
		BalanceMaxQuota: best.rule.BalanceMaxQuota,
	}
}
...
func matchBalanceRange(rule parsedDynamicRatioRule, balanceQuota int64) bool {
	if rule.BalanceMinQuota != nil && balanceQuota < *rule.BalanceMinQuota {
		return false
	}
	if rule.BalanceMaxQuota != nil && balanceQuota >= *rule.BalanceMaxQuota {
		return false
	}
	return true
}
