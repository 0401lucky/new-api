...
func migrateDB() error {
	...
	if err := normalizeSystemTaskActiveKeys(); err != nil {
		return err
	}
	...
	if err := MigrateDonations(DB); err != nil {
		return err
	}
...
// normalizeSystemTaskActiveKeys 在创建 active_key 唯一索引前清理旧版本数据。
// 旧版本可能为已结束任务保留非空 active_key，或同一类型存在多个 pending/running
// 任务；这里按类型只保留最新一条，避免 SQLite/MySQL/PostgreSQL 建索引失败。
func normalizeSystemTaskActiveKeys() error {
	if DB == nil || !DB.Migrator().HasTable(&SystemTask{}) {
		return nil
	}
	return DB.Transaction(func(tx *gorm.DB) error {
		if err := tx.Model(&SystemTask{}).
			Where("status IN ?", []SystemTaskStatus{SystemTaskStatusSucceeded, SystemTaskStatusFailed}).
			Updates(map[string]any{"active_key": nil, "locked_by": ""}).Error; err != nil {
			return err
		}
		legacyWithoutLockTable := !tx.Migrator().HasTable(&SystemTaskLock{})
		if legacyWithoutLockTable {
			if err := tx.Model(&SystemTask{}).Where("status = ?", SystemTaskStatusRunning).
				Updates(map[string]any{"status": SystemTaskStatusFailed, "active_key": nil, "locked_by": "", "error": "released during system task migration"}).Error; err != nil {
				return err
			}
		}

		var running []SystemTask
		if err := tx.Where("status = ?", SystemTaskStatusRunning).Order("id desc").Find(&running).Error; err != nil {
			return err
		}
		kept := make(map[string]bool, len(running))
		for _, task := range running {
			if kept[task.Type] {
				if err := tx.Model(&SystemTask{}).Where("id = ?", task.ID).Updates(map[string]any{"status": SystemTaskStatusFailed, "active_key": nil, "locked_by": "", "error": "deduplicated during system task migration"}).Error; err != nil {
					return err
				}
				continue
			}
		...
		if err := tx.Where("status = ?", SystemTaskStatusPending).Order("id desc").Find(&pending).Error; err != nil {
			return err
		}
		for _, task := range pending {
			if kept[task.Type] {
				if err := tx.Model(&SystemTask{}).Where("id = ?", task.ID).Updates(map[string]any{"status": SystemTaskStatusFailed, "active_key": nil, "error": "deduplicated during system task migration"}).Error; err != nil {
					return err
				}
				continue
			}
			if err := tx.Model(&SystemTask{}).Where("id = ?", task.ID).Updates(map[string]any{"active_key": task.Type}).Error; err != nil {
				return err
			}
			kept[task.Type] = true
		}
...
PRIMARY KEY (` + "`id`" + `)
	...
	required := []sqliteColumnDef{
		...
		{Name: "auto_grant", DDL: "`auto_grant` numeric DEFAULT 0"},
