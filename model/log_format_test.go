...
func TestRequestUserAgentStaysAdminOnlyWithoutMutatingOtherMetadata(t *testing.T) {
	other := NewLogOther()
	other.SetPublic("request_path", "/v1/chat/completions")
	other.SetAdmin("channel_name", "private-channel")
	c, _ := gin.CreateTestContext(httptest.NewRecorder())
	c.Request = httptest.NewRequest("POST", "/v1/chat/completions", nil)
	c.Request.Header.Set("User-Agent", strings.Repeat("客", 300))

	stored := otherWithRequestUserAgent(c, other)
	adminLogs := []*Log{{Other: stored.JSONString()}}
	FormatAdminLogs(adminLogs)
	fields, err := common.StrToMap(adminLogs[0].Other)
	require.NoError(t, err)
	adminInfo, ok := fields["admin_info"].(map[string]interface{})
	require.True(t, ok)
	assert.Equal(t, strings.Repeat("客", 256), adminInfo["user_agent"])
	assert.Equal(t, "private-channel", adminInfo["channel_name"])
	assert.NotContains(t, other.Snapshot()["admin_info"], "user_agent")

	userLogs := []*Log{{Other: stored.JSONString()}}
	formatUserLogs(userLogs, 0)
	assert.NotContains(t, userLogs[0].Other, "user_agent")
	assert.NotContains(t, userLogs[0].Other, "private-channel")
	assert.Contains(t, userLogs[0].Other, "request_path")
}
