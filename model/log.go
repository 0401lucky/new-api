...
func applyLogUserFilter(tx *gorm.DB, usernameColumn string, userIdColumn string, username string, userId int) (*gorm.DB, error) {
	if userId > 0 {
		return tx.Where(userIdColumn+" = ?", userId), nil
	}
	return applyExplicitLogTextFilter(tx, usernameColumn, username)
}
...
func otherWithRequestUserAgent(c *gin.Context, other *LogOther) *LogOther {
	...
	if other != nil {
		*merged = *other
		merged.adminInfo = copyLogOtherMap(other.adminInfo)
	}
	merged.SetAdmin("user_agent", userAgent)
	return merged
}
...
func recordErrorLog(c *gin.Context, userId int, channelId int, modelName string, tokenName string, content string, tokenId int, useTimeSeconds int,
	...
	otherStr := otherWithRequestUserAgent(c, other).JSONString()
	// 判断是否需要记录 IP
	needRecordIp := false
...
func RecordConsumeLog(c *gin.Context, userId int, params RecordConsumeLogParams) {
	...
	upstreamRequestId := c.GetString(common.UpstreamRequestIdKey)
	createdAt := common.GetTimestamp()
	otherStr := otherWithRequestUserAgent(c, params.Other).JSONString()
	// 判断是否需要记录 IP
	needRecordIp := false
	if settingMap, err := GetUserSetting(userId, false); err == nil {
		if settingMap.RecordIpLog {
...
func GetAllLogs(logType int, startTimestamp int64, endTimestamp int64, modelName string, username string, userId int, tokenName string, startIdx int, num int, channel int, group string, requestId string, upstreamRequestId string, promptCheckOnly bool) (logs []*Log, total int64, err error) {
	...
	if tx, err = applyLogUserFilter(tx, "logs.username", "logs.user_id", username, userId); err != nil {
		return nil, 0, err
	}
	if tokenName != "" {
...
func SumUsedQuota(logType int, startTimestamp int64, endTimestamp int64, modelName string, username string, userId int, tokenName string, channel int, group string) (stat Stat, err error) {
	...
	if rpmTpmQuery, err = applyLogUserFilter(rpmTpmQuery, "username", "user_id", username, userId); err != nil {
		return stat, err
	}
	if tokenName != "" {
