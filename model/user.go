	...
	if keyword != "" {
		if strings.HasPrefix(keyword, "#") {
			...
			if _, err := strconv.Atoi(keyword); err == nil {
				...
				likeArgs = append([]any{"%" + keyword + "%"}, likeArgs...)
			}
...
func DeleteUserById(id int) (err error) {
	...
	if err := DB.Transaction(func(tx *gorm.DB) error {
		return DeleteUserByIdWithTx(tx, id)
	}); err != nil {
		return err
	}
...
func DeleteUserByIdWithTx(tx *gorm.DB, id int) error {
	if id == 0 {
		return errors.New("id 为空！")
	}
	return (&User{Id: id}).DeleteWithTx(tx)
}
...
func HardDeleteUserByIdWithTx(tx *gorm.DB, id int) error {
	if id == 0 {
		return errors.New("id 为空！")
	}
	var tokens []Token
	...
	if err := invalidateTokensCache(tokens); err != nil {
		common.SysError(fmt.Sprintf("failed to invalidate token cache while hard deleting user %d: %v", id, err))
	}
	return nil
}
...
func (user *User) Delete() error {
	return user.delete(nil, nil)
}

func DeleteUserForSession(identity AuthSessionIdentity, cleanup ...func(*gorm.DB, int) error) error {
	user := User{Id: identity.UserID}
	var beforeDelete func(*gorm.DB, int) error
	if len(cleanup) > 0 {
		beforeDelete = cleanup[0]
	}
	return user.delete(&identity, beforeDelete)
}

func (user *User) delete(identity *AuthSessionIdentity, beforeDelete func(*gorm.DB, int) error) error {
	if user.Id == 0 {
		return errors.New("id 为空！")
	}
	...
	if err := DB.Transaction(func(tx *gorm.DB) error {
		...
		if beforeDelete != nil {
			if err := beforeDelete(tx, user.Id); err != nil {
				return err
			}
		}
		var err error
...
// DeleteWithTx 在调用方事务内软删除用户。删除前递增 auth_version 并发布
// pending fence，保证事务提交前后缓存读取都会回落到数据库并拒绝已删除用户。
func (user *User) DeleteWithTx(tx *gorm.DB) error {
	if user.Id == 0 {
		return errors.New("id 为空！")
	}
