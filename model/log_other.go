...
func formatLogOtherJSON(value string, visibility logOtherVisibility) string {
	...
	if visibility == logOtherVisibilityUser {
		if rawPromptCheck, exists := values["prompt_check"]; exists {
			var promptCheck map[string]json.RawMessage
			if err := common.Unmarshal(rawPromptCheck, &promptCheck); err == nil {
				if _, exists := promptCheck["full_text"]; exists {
					delete(promptCheck, "full_text")
					if encoded, err := common.Marshal(promptCheck); err == nil {
						values["prompt_check"] = encoded
						changed = true
					}
				}
			}
