...
function assertSuccess(res: ApiResponse, fallbackMessage: string): void {
  if (!res.success) {
    throw new Error(res.message || fallbackMessage)
  }
}
...
export async function setDynamicRatioEnabled(enabled: boolean): Promise<void> {
  ...
  assertSuccess(res.data, 'Failed to update dynamic ratio status')
}
...
export async function deleteDynamicRatioRule(id: number): Promise<void> {
  ...
  assertSuccess(res.data, 'Failed to delete dynamic ratio rule')
}
...
export async function reorderDynamicRatioRules(ids: number[]): Promise<void> {
  ...
  assertSuccess(res.data, 'Failed to reorder dynamic ratio rules')
}
