...
export function PrivacyCard(props: PrivacyCardProps) {
  ...
  const [leakProtectionEnabled, setLeakProtectionEnabled] = useState(
    () =>
      !parseUserSettings(props.profile.setting).disable_leak_protection_balanced
  )
  useEffect(() => {
    setRecordIpLog(
      Boolean(parseUserSettings(props.profile.setting).record_ip_log)
    )
    setLeakProtectionEnabled(
      !parseUserSettings(props.profile.setting).disable_leak_protection_balanced
    )
  ...
  const save = useMutation({
    mutationFn: async () => {
      const response = await updateUserSettings({
        ...
        disable_leak_protection_balanced: !leakProtectionEnabled,
      })
      if (!response.success) {
  ...
  return (
    <TitledCard title={t('Privacy')} disableHoverEffect>
      ...
      <div className='mt-4 flex items-center justify-between gap-4'>
        <div className='space-y-1'>
          <Label htmlFor='security-leak-protection'>
            {t('Leak protection')}
          </Label>
          <p className='text-muted-foreground text-sm'>
            {t(
        ...
        <Switch
          id='security-leak-protection'
          checked={leakProtectionEnabled}
          onCheckedChange={setLeakProtectionEnabled}
          disabled={save.isPending}
        />
      </div>
