...
describe('API key group table cell', () => {
  ...
  test('renders the unclipped Auto ratio ring and a localized Auto ratio when API data uses a nonlocalized string', async () => {
  ...
  // AutoGroupBadge is currently commented out in ApiKeyGroupCell (upstream
  // 0ab020206), so an Auto row without ratio data renders no Auto frame at all.
  test('renders no Auto frame when ratio data is unavailable', async () => {
    const container = document.createElement('div')
    document.body.append(container)
    const root = createRoot(container)
