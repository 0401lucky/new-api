...
function renderPlans(
  ...
  return render(<SubscriptionPlansCard topupInfo={null} userQuota={5_000_000} />)
}
...
describe('subscription purchase limit', () => {
  ...
  ])('allows renewal when the previous subscription is %s', async (_, overrides) => {
    renderPlans([subscription(overrides)])
    const user = userEvent.setup()
    await user.click(await screen.findByRole('button', { name: 'Subscribe Now' }))
    const dialog = await screen.findByRole('dialog', { name: 'Purchase Subscription' })
    expect(within(dialog).getByRole('button', { name: 'Pay with Balance' })).toBeEnabled()
  })

  it.each([0, 1000])('blocks another purchase while the subscription is active with %i quota used', async (used) => {
  ...
  it('allows a remaining active slot despite expired history and another plan', async () => {
    ...
    expect(await screen.findByRole('button', { name: 'Subscribe Now' })).toBeEnabled()
  })
